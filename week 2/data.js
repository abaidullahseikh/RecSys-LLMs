...
// Known genres: raw MovieLens offsets 1-18 (offset 0 is unknown).
const genreNames = [
...
async function loadData() {
    try {
        ...
        // MovieLens 100K stores u.item as Latin-1 rather than UTF-8.
...
function parseItemData(text) {
    ...
    for (const line of lines) {
        ...
        if (fields.length < 24) continue; // Require metadata and all 19 flags
        
        const id = parseInt(fields[0]);
        const title = fields[1];
        
        // Keep unknown separately; similarity uses all 18 known genres, including Western.
        const unknownGenre = fields[5] === '1';
        const genreVector = fields.slice(6, 24).map(Number);
        if (genreVector.some(value => value !== 0 && value !== 1)) continue;
        const genres = genreNames.filter((_, index) => genreVector[index] === 1);
        
        movies.push({ id, title, genres, genreVector, unknownGenre });
    }
}
